"""
基于 notes 表数据，预计算各视频号场景的 baseline 统计指标并写入 baseline_stats 表。
...
def compute_for_category(cursor, category):
    ...
    # --- 互动数据：当前库表用 collects 存放收藏，可作为转发/收藏倾向的近似指标 ---
    ...
    print(f"  [{category}] 已计算视频号 baseline 指标（含账号分层与关键词分桶）")


def main():
    """计算所有视频号场景的 baseline 统计指标"""
    conn = sqlite3.connect(DB_PATH)
    cursor = conn.cursor()

    cursor.execute("DELETE FROM baseline_stats")

    for cat in ["local", "knowledge", "emotion", "enterprise", "shop", "ip"]:
        compute_for_category(cursor, cat)

    conn.commit()
    conn.close()
    print("所有视频号 baseline 统计指标已计算完毕")
