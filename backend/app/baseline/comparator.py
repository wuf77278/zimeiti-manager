"""
Baseline 对比引擎
将用户视频号内容的各项特征与对应场景的 baseline 数据进行量化对比。
"""
import sqlite3
import json

from app.paths import baseline_db_path

DB_PATH = str(baseline_db_path())


class BaselineComparator:
    """对比用户视频号内容特征与 baseline 数据"""
    ...
    def get_category_stats(self, category: str) -> dict:
        """
        获取指定视频号场景的全部 baseline 统计数据。

        @param category - 场景标识 (local / knowledge / emotion / enterprise / shop / ip)
        @returns dict 所有统计指标
        """
        conn = self._get_conn()
        cursor = conn.cursor()
    ...
    def compare(self, category: str, note_features: dict) -> dict:
        """
        将视频号内容特征与 baseline 对比，返回各维度的偏差分析。

        @param category - 视频号场景
        @param note_features - 内容分析结果 (title_analysis + content_analysis + image_analysis)
        ...
        keyword_count = note_features.get("tag_count", 0)
        avg_keyword_count = stats.get("avg_tag_count", 0)
        comparisons["keyword_count"] = {
            "user_value": keyword_count,
            "category_avg": avg_keyword_count,
            "deviation": round(keyword_count - avg_keyword_count, 1),
            "verdict": "偏少" if keyword_count < avg_keyword_count * 0.7 else (
                "偏多" if keyword_count > avg_keyword_count * 1.5 else "合适"
            ),
        }

        top_keywords = stats.get("top_tags", [])
        user_keywords = note_features.get("tags", [])
        hot_keyword_names = {t["tag"] for t in top_keywords[:10]} if isinstance(top_keywords, list) else set()
        matched = [t for t in user_keywords if t in hot_keyword_names]
        comparisons["scenario_keyword_relevance"] = {
            "matched_hot_keywords": matched,
            "hot_keyword_coverage": round(len(matched) / max(len(user_keywords), 1), 2),
            "top_keywords_in_category": [t["tag"] for t in top_keywords[:10]] if isinstance(top_keywords, list) else [],
        }
        ...
        if "has_face" in note_features:
            ...
            comparisons["cover_face"] = {
                ...
                "suggestion": "建议真人出镜或强化使用场景" if face_rate > 50 and not note_features["has_face"] else "",
            }
        ...
        comparisons["avg_interaction_proxy"] = avg_engagement
        ...
        comparisons["viral_interaction_proxy"] = viral_engagement
