...
logger = logging.getLogger("videorx.video_analyzer")
...
class VideoAnalyzer:
    ...
    async def analyze(
        ...
        *,
        ...
        user_prompt = (
            "Analyze the uploaded video for WeChat Video Channels content diagnosis and return JSON with fields: "
            "summary (string, describe topic, target viewer, and use scenario), "
            "scene_keywords (array of <=8 strings), "
            "opening_hook (string, summarize the first 3 seconds if visible/audible), "
            "retention_risks (array of strings), "
            "sharing_angle (string, why someone might forward it to Moments, a group, family, colleagues, or customers), "
            "cover_suggestion (string), "
            "has_face (boolean), "
            "shot_style (string), "
            "risk_or_limitations (array of strings). "
            "If confidence is low, still return best-effort values and do not invent unseen text."
        )
