"""
OCR 处理模块
使用 mimo-v2-omni 多模态模型提取视频号截图中的内容信息。
"""
...
logger = logging.getLogger("videorx.ocr")
...
class OCRProcessor:
    ...
    async def extract_text(
        ...
        *,
        ...
        try:
            msg_body: list | str = [
                {"type": "text", "text": "请基于视频号截图语义提取视频标题、发布文案/字幕/口播要点和话题关键词；无需逐字 OCR 整屏。"},
                ...
                "messages": [
                    {
                        ...
                        "content": (
                            ...
                            "title 只填写画面明确出现的视频标题、封面大字或发布页标题。"
                            "content 只写可见发布文案、字幕、口播要点、话题或承接信息，一句一行用 \\n；避免长篇场景描述以控制长度。"
                            "tags 提取话题、行业、场景或关键词，无则 []。"
