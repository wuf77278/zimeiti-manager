...
router = APIRouter()
logger = logging.getLogger("videorx.screenshot")
...
_QUICK_PROMPT = """分析这张微信视频号截图，判断素材类型并提取可见文字。

## slot_type 判断规则
- cover：视频封面、首帧或信息流卡片截图，一张大图/视频画面为主
- content：发布页、作品详情页或正文区域，包含视频标题、简介/发布文案、话题或位置等信息
- comments：多条评论列表（头像+昵称+评论文字）
- profile：视频号主页，包含头像、昵称、简介、粉丝/关注数据和作品网格
- other：以上都不是

## 提取规则
- title：仅在画面明确出现视频标题、发布页标题或封面大字时提取；否则留空""
- content_text：提取可见简介、发布文案、话题、字幕或口播关键句；其他类型留空""
- category：民宿酒店设计/办公空间设计/设计案例复盘/业主避坑/设计科普
- summary：1句概括这条视频号素材的内容和场景
...
_VIDEO_QUICK_PROMPT = """你是微信视频号内容理解助手。用户上传了一段设计公司运营素材（录屏、口播、Vlog、空间案例、项目讲解等）。

## 关于 title（极其重要）
视频号标题通常来自发布页标题、封面大字或视频卡片文案，**不等同于视频里在讲什么**。
- title 默认留空 ""。只有画面里明确出现发布标题、封面主标题或卡片标题时才填写。
- 禁止把画面说明、步骤提示、口播摘要、贴纸花字、"视频展示…""画面中…"类描述句写入 title。
- 用户若要自动填标题，应上传含标题/封面大字的截图；本接口主要理解视频内容。

## content_text = 字幕/口播/画面文案摘录（供后续诊断，极其重要）
你必须通篇观看整段视频（从开头到结尾），不能只依据前几秒或片段猜测。
- 按时间顺序摘录视频中出现的：字幕条、花字、贴纸、角标、弹窗文字、口播关键句。
- 尽量逐字摘录，尤其保留开头3秒、转折、案例、结尾互动引导。
- 禁止用"视频展示了…""画面中一位…""本视频主要讲…"这类元描述充当正文主体；如需总览，只放在 summary。
- 话题 #xxx、空间类型、项目名、公众号/案例/企微承接信息若出现，按出现顺序并入 content_text。
- 严禁编造；听不清/看不清处用 `[无法辨识]` 占位，可注明大致时段。
- 同一句字幕/口播明显重复多次时，可合并为一句并注明"（重复）"。

## 其它字段
1) slot_type：多为 content；整屏主页 profile；几乎只有评论列表 comments；否则 other。
2) extra_slots：规则同截图快识。
3) category：民宿酒店设计/办公空间设计/设计案例复盘/业主避坑/设计科普。
4) summary：仅 1～2 句整体提要，说明内容主题、目标用户和使用场景；不得替代 content_text。
5) confidence：0～1，反映你对字幕摘录完整度与准确度的把握。

## 错误示例 vs 正确示例（必须遵守）
- 错误 content_text："视频展示了一位老师在讲家庭沟通方法。"（这是旁白式说明，禁止）
- 正确 content_text："如果你总是忍不住吼孩子\n先别急着讲道理\n这三个信号说明他其实没听懂"（逐字来自画面/口播）
...
_DEEP_PROMPT_COVER = """分析这张视频号封面/首帧截图的点击力，输出 JSON：
{"visual_score": 0-100, "color_scheme": "配色描述", "composition": "构图评价", "text_overlay": "封面大字/字幕可读性评价", "suggestions": ["建议1", "建议2"]}"""

_DEEP_PROMPT_CONTENT = """提取这张视频号发布文案/作品详情截图中的关键信息，输出 JSON：
{"title": "视频标题", "content": "发布文案、字幕或口播要点", "tags": ["话题或关键词1"], "word_count": 数字, "readability": "视频号语境下的可读性评价"}"""

_DEEP_PROMPT_PROFILE = """分析这张视频号主页截图，输出 JSON：
{"nickname": "昵称", "follower_count": "粉丝数文本", "note_count": "作品数", "bio": "简介", "account_level": "素人/成长中/成熟账号", "niche": "视频号场景领域"}"""

_DEEP_PROMPT_COMMENTS = """分析这张视频号评论区截图中的评论，输出 JSON：
{"comments": [{"text": "评论内容", "sentiment": "positive|negative|neutral"}], "overall_sentiment": "整体情感倾向", "engagement_quality": "互动质量评价", "top_concerns": ["用户关心点1"]}"""
...
def _sanitize_video_derived_title(result: dict) -> None:
    """
    视频快识易把「画面描述」误填进 title。清空并并入 content_text，避免与真实视频标题混淆。
    """
...
def _build_overall(analyses: dict, scenario: str) -> dict:
    ...
    if not has_content:
        tips.append("缺少发布文案截图，无法分析标题、脚本与正文信息")
