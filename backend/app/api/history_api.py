...
router = APIRouter()
logger = logging.getLogger("videorx.history")
    ...
    try:
        conn.execute(
            """INSERT INTO diagnosis_history
               (id, title, category, platform, overall_score, grade, report_json)
               VALUES (?, ?, ?, ?, ?, ?, ?)""",
            (record_id, req.title, req.category, req.platform, overall_score, grade, json.dumps(report, ensure_ascii=False)),
        )
        conn.commit()
...
async def list_history(
    ...
    try:
        rows = conn.execute(
            """SELECT id, title, category, platform, overall_score, grade, created_at
    ...
    return [
        HistoryListItem(
            ...
            title=r["title"],
            category=r["category"],
            platform=r["platform"] or "wechat_channels",
...
async def get_history(record_id: str):
    ...
    try:
        row = conn.execute(
            """SELECT id, title, category, platform, overall_score, grade, report_json, created_at
    ...
    return HistoryDetail(
        ...
        platform=row["platform"] or "wechat_channels",
