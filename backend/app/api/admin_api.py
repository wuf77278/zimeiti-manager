...
from app.config import require_admin_token
from app.paths import baseline_db_path

router = APIRouter()
logger = logging.getLogger("zimeiti.admin")
...
function showLogin(err){
  app.innerHTML=`<div class="login"><div class="login-box"><h1>自媒体管家 Admin</h1><p>请输入本地 ADMIN_TOKEN</p>
  <input type="password" id="pw" placeholder="访问令牌" onkeydown="if(event.key==='Enter')doLogin()">
  <button onclick="doLogin()">进入</button>${err?'<div class=err>'+err+'</div>':''}</div></div>`;
  document.getElementById('pw')?.focus();
}
async function doLogin(){
  const pw=document.getElementById('pw').value;
  try{const r=await fetch('/admin/api/stats',{headers:{Authorization:'Bearer '+pw}});
  if(!r.ok){showLogin(r.status===401?'请输入访问令牌':'访问令牌无效');return;}token=pw;showDash(await r.json());}catch(e){showLogin('连接失败');}
}
...
function showDash(d){
  ...
  <h1>自媒体管家 Admin</h1><div class="sub">${d.timestamp} · uptime ${Math.round(d.uptime_seconds/60)}min</div>
  <div class="g">
    ...
    <div class="c"><div class="l">训练样本</div><div class="v sm">${d.total_notes||0}</div></div>
  ...
  <table><tr><th>时间</th><th>IP</th><th>标题</th><th>场景</th><th>Token</th><th>耗时</th><th>状态</th></tr>
...
async function doRefresh(){
  if(!token)return;
  try{const r=await fetch('/admin/api/stats',{headers:{Authorization:'Bearer '+token}});
...
@router.get("/admin/api/stats", dependencies=[Depends(require_admin_token)])
