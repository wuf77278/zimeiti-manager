...
logger = logging.getLogger("videorx.orchestrator")
...
def _build_stable_scores(
    ...
    """
    从 Model A 预评分 + 文本/图像/视频号运营上下文构建确定性雷达分数。
    不依赖 LLM 输出 → 同一输入永远产出相同分数。
    """
    ...
    opening_bonus = 8.0 if opening_hook.strip() else 0.0
    script_bonus = 8.0 if len(script.strip()) >= 80 else (4.0 if script.strip() else 0.0)

    title_hook_score = _clamp_score(title_quality * 0.78 + opening_bonus + content_quality * 0.12)
    script_retention_score = _clamp_score(
        content_quality * 0.42 + readability * 0.18 + info_density * 0.18 + script_bonus + opening_bonus
    )

    if image_analysis:
        saturation = float(image_analysis.get("saturation", 0)) * 100
        text_ratio = float(image_analysis.get("text_ratio", 0))
        text_balance = max(0.0, 100.0 - abs(text_ratio - 0.24) * 240.0)
        face_seen = bool(image_analysis.get("has_face")) or bool((video_analysis or {}).get("has_face"))
        face_bonus = 8.0 if face_seen else 0.0
        cover_click_score = _clamp_score(
            visual_quality * 0.62 + saturation * 0.14 + text_balance * 0.18 + face_bonus
        )
    elif video_analysis:
        face_bonus = 8.0 if video_analysis.get("has_face") else 0.0
        cover_click_score = _clamp_score(visual_quality * 0.82 + 10.0 + face_bonus)
    else:
        cover_click_score = _clamp_score(visual_quality)
    ...
    interaction_rate = ((shares * 1.4 + comments * 1.2 + likes) / plays * 100) if plays > 0 else 0.0
    completion_bonus = completion_rate if completion_rate <= 100 else completion_rate * 100
    interaction_share_score = _clamp_score(
        engagement_potential * 0.52 + title_hook_score * 0.18 + script_retention_score * 0.12
        + min(interaction_rate * 12, 14) + min(completion_bonus * 0.12, 12)
    )
    positioning_fit_score = _clamp_score(
        title_hook_score * 0.26 + script_retention_score * 0.24 + interaction_share_score * 0.30
        + content_quality * 0.20
    )
    raw_overall = (
        title_hook_score + cover_click_score + script_retention_score
        + interaction_share_score + positioning_fit_score
    ) / 5
    model_a_overall = float(model_a_score.get("total_score", 50))
    overall_score = _clamp_score(model_a_overall * 0.35 + raw_overall * 0.65)

    return {
        "title_hook": title_hook_score,
        "cover_click": cover_click_score,
        "script_retention": script_retention_score,
        "interaction_share": interaction_share_score,
        "positioning_fit": positioning_fit_score,
...
class Orchestrator:
    ...
    async def run(
        ...
        t0 = time.time()
        metrics = metrics or {}
        full_text_for_analysis = "\n\n".join(part for part in [content, opening_hook, script] if part)
        ...
        # --- Step 1: 多模态内容解析 ---
        await _emit_progress("parse_start", "正在解析标题、脚本与视频号素材...")
        title_analysis = self.text_analyzer.analyze_title(title)
        content_analysis = self.text_analyzer.analyze_content(full_text_for_analysis or content)
        ...
        # --- Step 2: Baseline 对比 ---
        await _emit_progress("baseline_start", "正在进行视频号同类基线对比与预评分...")
        note_features = {
        ...
        await _emit_progress("round1_start", "4位视频号专家正在并行诊断...")
        t1 = time.time()
        ...
        round1_tasks = [
            _run_round1_agent(
                ...
                content_agent.diagnose(
                    ...
                    script=script, opening_hook=opening_hook,
                    publish_time=publish_time, metrics=metrics,
                ),
            ),
            _run_round1_agent(
                "封面视觉专家",
    ...
    async def _run_debate(
        ...
        agent_names = ["标题脚本", "封面视觉", "增长运营", "观众模拟"]
        debate_records = []
        debate_tokens = 0
    ...
    def _assemble_response(self, final_report, agent_opinions, simulated_comments, debate_timeline, stable_scores=None) -> dict:
        ...
        if stable_scores:
            ...
            if not radar:
                ...
                title_script = scores.get("标题与脚本留存", 50)
                radar = {
                    "title_hook": title_script,
                    "cover_click": scores.get("封面点击力", 50),
                    "script_retention": title_script,
                    "interaction_share": scores.get("互动转发力", 50),
                    "positioning_fit": scores.get("观众反应", 50),
        ...
        return {
            ...
            "workflow_trace": build_noterx_workflow_trace("wechat_channels"),
        }
