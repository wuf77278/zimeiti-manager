...
Analyzes Video Channels cover/first-frame click performance.
"""
...
logger = logging.getLogger("videorx.visual_agent")
...
class VisualAgent(BaseAgent):
    ...
    def build_user_message(
        ...
        """Build prompt with image/video analysis and baseline comparison."""
        comparisons = baseline_comparison.get("comparisons", {})
        metrics = metrics or {}
        content_data = build_content_data_summary("wechat_channels", category, agent_type="visual", max_items=3)

        image_parts: list[str] = []
        if image_analysis:
            image_parts.append(f"""## 封面/首帧图像分析
...
- 风险/限制: {json.dumps(video_analysis.get('risk_or_limitations', []), ensure_ascii=False)}""")
        if image_parts:
            ...
            image_info = "## 封面/首帧图像分析\n未收到封面或首帧图片，请基于标题和垂类给出封面建议。"
...
请基于视频号信息流、朋友圈转发卡片和微信群预览场景，给出封面/首帧视觉诊断。"""
    ...
    async def diagnose(
        ...
        *,
        ...
        vision_tail = (
            "\n\n## 任务说明\n"
            "已附上视频号封面/首帧图。请直接观察画面中的主体、文字、色彩、构图、人物/产品与背景，"
            "结合上文量化指标与 Baseline 输出严格 JSON；reasoning 中必须引用你看到的具体画面元素。"
        )
