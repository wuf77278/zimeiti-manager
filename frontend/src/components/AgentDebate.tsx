...
const AGENT_COLORS: Record<string, { bg: string; accent: string; text: string }> = {
  ...
  "秒停留 Agent": { bg: "#fff5f6", accent: "#ff2442", text: "#dc2626" },
  "完播 Agent": { bg: "#fffbeb", accent: "#f59e0b", text: "#d97706" },
  "流量场 Agent": { bg: "#eff6ff", accent: "#3b82f6", text: "#2563eb" },
  "设计字段 Agent": { bg: "#eff6ff", accent: "#3b82f6", text: "#2563eb" },
  "转化承接 Agent": { bg: "#f0fdf4", accent: "#10b981", text: "#059669" },
};
...
function agentInitial(name: unknown): string {
  ...
  return (
    <Stack spacing={2}>
      ...
      {opinions.map((op, idx) => {
        ...
        const agentName = safeText(op.agent_name, "诊断 Agent");
        const dimension = safeText(op.dimension, "综合诊断");
        const issues = safeList(op.issues);
        const suggestions = safeList(op.suggestions);
        const colors = AGENT_COLORS[agentName] || { bg: "#f9f9f9", accent: "#666", text: "#333" };
        const score = Number.isFinite(Number(op.score)) ? Number(op.score) : 0;
          ...
          <Box key={idx}>
            <Box onClick={() => setExpandedIdx(isOpen ? null : idx)} sx={{
              ...
              <Box sx={{ width: 32, height: 32, borderRadius: "8px", flexShrink: 0,
                ...
                <Typography sx={{ color: "#fff", fontSize: 13, fontWeight: 700 }}>{agentInitial(agentName)}</Typography>
              </Box>
              <Box sx={{ flex: 1, minWidth: 0 }}>
                <Typography sx={{ fontWeight: 600, fontSize: 13, color: "#262626" }}>{agentName}</Typography>
                <Typography sx={{ fontSize: 11, color: "#999" }}>{dimension}</Typography>
              </Box>
              <Typography sx={{ fontWeight: 800, fontSize: 16, color: scoreColor }}>{Math.round(score)}</Typography>
        ...
        <Box>
          ...
          <Stack spacing={1}>
            {(showAllTimeline ? timeline : timeline.slice(0, 3)).map((entry, i) => {
              ...
              const agentName = safeText(entry.agent_name, "诊断 Agent");
              const entryText = safeText(entry.text, "");
              const colors = AGENT_COLORS[agentName] || { accent: "#666", bg: "#f9f9f9", text: "#333" };
              return (
                <Box key={i} sx={{ display: "flex", gap: 1, alignItems: "flex-start" }}>
                  <Box sx={{
                    ...
                    <Typography sx={{ color: "#fff", fontSize: 10, fontWeight: 700 }}>{agentInitial(agentName)}</Typography>
                  </Box>
                  <Box sx={{
                    ...
                    <Box sx={{ display: "flex", alignItems: "center", gap: 0.5, mb: 0.25 }}>
                      <Typography sx={{ fontSize: 11, fontWeight: 600, color: "#262626" }}>{agentName}</Typography>
                      <Box sx={{ fontSize: 9, fontWeight: 700, color: kind.color, bgcolor: `${kind.color}12`, borderRadius: "4px", px: 0.5, py: 0.1 }}>
                        {kind.label}
                      </Box>
                    </Box>
                    <Typography sx={{ fontSize: 12, color: "#555", lineHeight: 1.6 }}>{entryText}</Typography>
                  </Box>
                </Box>
              );
            })}
