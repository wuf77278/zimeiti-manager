...
function avatarColor(name: unknown = "用户"): string {
  const safeName = String(name || "用户");
  let hash = 0;
  for (let i = 0; i < safeName.length; i++) hash = safeName.charCodeAt(i) + ((hash << 5) - hash);
  return AVATAR_COLORS[Math.abs(hash) % AVATAR_COLORS.length];
}

function avatarInitial(name: unknown = "用户"): string {
  return String(name || "用户").charAt(0) || "?";
}
...
function normalizeComment(c: SimulatedComment | CommentWithReplies): SimulatedComment | CommentWithReplies {
  const raw = c as SimulatedComment & { content?: string; sentiment?: string; type?: string; username?: unknown; comment?: unknown };
  return {
    ...c,
    username: String(raw.username || (raw.type === "target_user" ? "目标用户" : raw.sentiment === "positive" ? "潜在粉丝" : "路人用户")),
    comment: String(raw.comment || raw.content || "这条内容我会想继续看。"),
  };
}

function toCommentState(c: SimulatedComment | CommentWithReplies): CommentState {
  const normalized = normalizeComment(c);
  const replies = ("replies" in normalized && Array.isArray(normalized.replies) ? normalized.replies : []).map((r) => {
    const nr = normalizeComment(r) as SimulatedComment;
    return {
      ...nr,
      _likes: nr.likes ?? Math.floor(Math.random() * 80),
      _liked: false,
    };
  });
  return {
