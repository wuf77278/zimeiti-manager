    ...
    <Stack spacing={2}>
      {metrics.filter((m) => m.userValue > 0).map((m) => (
        <Box key={m.label}>
          ...
          <Stack spacing={0.75}>
            <BarRow label="你的作品" value={m.userValue} maxVal={maxVal} color="#ff2442" unit={m.unit} />
            <BarRow label="场景平均" value={m.avgValue} maxVal={maxVal} color="#ddd" unit={m.unit} />
            {m.viralValue !== undefined && m.viralValue > 0 && (
              <BarRow label="高表现平均" value={m.viralValue} maxVal={maxVal} color="#f59e0b" unit={m.unit} />
            )}
          </Stack>
        </Box>
      ))}
