...
export default function History() {
  ...
  const handleOpen = async (item: HistoryListItem) => {
    ...
    try {
      ...
      const tags = p.tags;
      const platform = (rec.report.platform || rec.platform || p.platform || "wechat_channels") as PlatformKey;
      navigate("/report", {
        state: {
          report: rec.report,
          params: { title, category, content, tags, platform },
          isFallback: false,
        },
      });
    } catch (e) {
          ...
          >
            <Box sx={{ display: "flex", flexDirection: "column", gap: 1.5 }}>
              {items.map((item) => {
                ...
                const platform = (item.platform || "wechat_channels") as PlatformKey;
